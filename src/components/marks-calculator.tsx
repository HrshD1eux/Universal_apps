...
export default function MarksCalculator() {
  ...
  const [result, setResult] = useState<any>({ total: 0, percentage: 0, grade: 'F', max: 0 });
  ...
  const handleCalculate = async () => {
    ...
    try {
        ...
        const res = await safeInvoke<any>('calculate_marks', {
            marks,
            maxMarks: max_marks
        });
        ...
        setResult({
            ...res,
            max: max_marks.reduce((a, b) => a + b, 0)
        });
    } catch (err) {
        // Silently fail or minimal log to avoid spamming
    }
  };

  useEffect(() => {
    handleCalculate();
  }, [subjects, totalMarksObtained, totalMaxMarks, activeTab]);

  const { total: totalMarks, max: finalTotalMaxMarks, percentage, grade } = result;
                        ...
                        <div className="overflow-x-auto rounded-xl border border-primary/10">
                            <Table>
                                ...
                                <TableRow className="hover:bg-transparent border-b-primary/20">
                                    ...
                                    <TableHead className="min-w-[120px]">Marks Obtained</TableHead>
                                    <TableHead className="min-w-[120px]">Max Marks</TableHead>
                                ...
                                {subjects.map((subject) => (
                                    ...
                                    <TableCell>
                                        <Button variant="ghost" size="icon" onClick={() => removeSubject(subject.id)} disabled={subjects.length <= 1} className="h-8 w-8">
