...
use chrono::{Datelike, NaiveDate, Duration, Timelike};
    ...
    match eval_str(&expression) {
        Ok(res) => {
            ...
            Ok(CalculationResult {
                result: res,
                formatted: format!("{:.10}", res).trim_end_matches('0').trim_end_matches('.').to_string(),
...
pub fn calculate_compound_interest(
    ...
    let final_amount_f64 = principal * (1.0 + rate / 100.0 / frequency as f64).powf(time * frequency as f64);
...
#[tauri::command]
pub fn calculate_age(dob: String, birth_time: Option<String>, now_date: Option<String>) -> Result<AgeResult, String> {
    ...
    let birth_dt = if let Some(time_str) = birth_time {
        let time = chrono::NaiveTime::parse_from_str(&time_str, "%I:%M %p").map_err(|e| format!("Time error: {}", e))?;
    ...
    let next_bday = NaiveDate::from_ymd_opt(next_bday_year, birth.month(), birth.day())
        .unwrap_or(NaiveDate::from_ymd_opt(next_bday_year, 3, 1).unwrap().pred_opt().unwrap()); // handle Feb 29
...
pub fn convert_units(request: ConvertRequest) -> Result<ConvertResult, String> {
    let factor = match request.category.to_lowercase().as_str() {
        ...
        "temperature" => return convert_temperature(request.value, &request.from_unit, &request.to_unit),
...
fn get_length_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ...
        ("micrometer", 0.000001), ("nanometer", 0.000000001), ("picometer", 1e-12), ("fentometer", 1e-15),
        ("angstrom", 1e-10), ("inch", 0.0254), ("foot", 0.3048), ("yard", 0.9144), ("mile", 1609.344),
        ("nautical_mile", 1852.0), ("lightyear", 9.4607304725808e15), ("parsec", 3.08567758149137e16),
        ("au", 1.495978707e11), ("furlong", 201.168), ("chain", 20.1168), ("rod", 5.0292),
        ("hand", 0.1016), ("span", 0.2286), ("league", 4828.032), ("micron", 1e-6), ("smoot", 1.7018),
        ("thou", 0.0000254), ("caliper", 0.0000254), ("link", 0.201168), ("pica", 0.00423333333),
        ("point", 0.000352777778), ("plank_length", 1.616255e-35), ("ell", 1.143)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
    Ok(from_f / to_f)
}

fn get_weight_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("kilogram", 1.0), ("gram", 0.001), ("milligram", 0.000001), ("microgram", 1e-9),
        ("metric_ton", 1000.0), ("quintal", 100.0), ("pound", 0.45359237), ("ounce", 0.028349523),
        ("stone", 6.35029318), ("carat", 0.0002), ("grain", 0.00006479891), ("slug", 14.5939029),
        ("troy_ounce", 0.0311034768), ("troy_pound", 0.3732417216), ("pennyweight", 0.00155517384),
        ("amu", 1.6605390666e-27), ("solar_mass", 1.98847e30), ("dram", 0.0017718452),
        ("quarter", 12.7005864), ("hundredweight_us", 45.359237), ("hundredweight_uk", 50.8023454)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
    Ok(from_f / to_f)
...
fn get_area_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("square_meter", 1.0), ("square_kilometer", 1e6), ("square_centimeter", 0.0001), ("square_millimeter", 1e-6),
        ("hectare", 10000.0), ("acre", 4046.8564224), ("square_mile", 2589988.11), ("square_yard", 0.83612736),
        ("square_foot", 0.09290304), ("square_inch", 0.00064516), 
        ("killa", 4046.85642), ("kanal", 505.85705), ("marla", 25.2928526), ("bigha", 2529.28526),
        ("kaccha_bigha", 843.095), ("biswa", 126.46426), ("biswansi", 6.323213), ("sq_karam", 2.8103166),
        ("guntha", 101.17141056), ("ground", 222.96751173), ("cent", 40.46856422), ("are", 100.0),
        ("chatak", 4.1806371), ("cottah", 66.890333), ("decimal", 40.468564), ("rood", 1011.7141), ("perch", 25.29285), ("section", 2589988.11)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
    Ok(from_f / to_f)
...
fn get_volume_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("liter", 1.0), ("milliliter", 0.001), ("cubic_meter", 1000.0), ("cubic_centimeter", 0.001), ("cubic_millimeter", 1e-6),
        ("us_gallon", 3.785411784), ("us_quart", 0.946352946), ("us_pint", 0.473176473), ("us_cup", 0.2365882365), ("us_fluid_ounce", 0.0295735295625),
        ("us_tablespoon", 0.01478676478125), ("us_teaspoon", 0.00492892159375),
        ("imp_gallon", 4.54609), ("imp_quart", 1.1365225), ("imp_pint", 0.56826125), ("imp_cup", 0.284130625), ("imp_fluid_ounce", 0.0284130625),
        ("imp_tablespoon", 0.017758164), ("imp_teaspoon", 0.005919388),
        ("barrel_oil", 158.987294928), ("hogshead", 238.481), ("bushel_us", 35.23907), ("peck_us", 8.8097675),
        ("gill_us", 0.118294118), ("dash", 0.0006), ("pinch", 0.0003), ("drop", 0.00005), ("minim", 0.0000616115), ("board_foot", 2.359737)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
    Ok(from_f / to_f)
...
fn get_time_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("second", 1.0), ("millisecond", 0.001), ("microsecond", 1e-6), ("nanosecond", 1e-9), ("picosecond", 1e-12),
        ("minute", 60.0), ("hour", 3600.0), ("day", 86400.0), ("week", 604800.0), ("fortnight", 1209600.0),
        ("month_avg", 2629746.0), ("year_julian", 31557600.0), ("decade", 315576000.0), ("century", 3155760000.0), ("millennium", 31557600000.0)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
    Ok(from_f / to_f)
...
fn get_data_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("bit", 0.125), ("byte", 1.0), ("kilobit", 125.0), ("kilobyte", 1000.0), ("megabit", 125000.0), ("megabyte", 1e6),
        ("gigabit", 1.25e8), ("gigabyte", 1e9), ("terabit", 1.25e11), ("terabyte", 1e12), ("petabit", 1.25e14), ("petabyte", 1e15),
        ("kibibyte", 1024.0), ("mebibyte", 1048576.0), ("gibibyte", 1073741824.0), ("tebibyte", 1099511627776.0)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
    Ok(from_f / to_f)
...
fn get_speed_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("mps", 1.0), ("kmph", 0.277777778), ("mph", 0.44704), ("knot", 0.514444444), ("mach", 340.3), ("light", 299792458.0),
        ("fps", 0.3048)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
...
fn get_energy_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("joule", 1.0), ("kilojoule", 1000.0), ("calorie", 4.184), ("kilocalorie", 4184.0), ("watt_hour", 3600.0), ("kilowatt_hour", 3.6e6),
        ("btu", 1055.05585), ("electronvolt", 1.602176634e-19), ("foot_pound", 1.35581794833)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
    Ok(from_f / to_f)
...
fn get_power_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("watt", 1.0), ("kilowatt", 1000.0), ("megawatt", 1e6), ("horsepower", 745.699872), ("btu_hr", 0.293071),
...
fn get_pressure_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("pascal", 1.0), ("bar", 100000.0), ("psi", 6894.75729), ("atmosphere", 101325.0), ("torr", 133.322),
        ("kpa", 1000.0), ("mpa", 1e6), ("inhg", 3386.388)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
    Ok(from_f / to_f)
...
fn get_force_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("newton", 1.0), ("kilonewton", 1000.0), ("dyne", 1e-5), ("lbf", 4.448222)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
    Ok(from_f / to_f)
...
fn get_angle_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("degree", 1.0), ("radian", 57.2957795), ("gradian", 0.9), ("arcminute", 1.0/60.0), ("arcsecond", 1.0/3600.0), ("revolution", 360.0)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
    Ok(from_f / to_f)
...
pub fn calculate_mortgage(
    ...
    while balance > 0.1 && months < 600 { // Limit to 50 years max
...
pub fn base_convert(value: String, from_base: u32, to_base: u32) -> Result<String, String> {
    ...
    let decimal = u64::from_str_radix(clean_value, from_base).map_err(|e| format!("Invalid input for base {}: {}", from_base, e))?;
...
pub fn calculate_date_math(
    ...
    if mode == "diff" {
        ...
        let (smaller, larger) = if start <= end { (start, end) } else { (end, start) };
        ...
        if d < 0 {
            m -= 1;
            let last_month = if larger.month() == 1 { 12 } else { larger.month() - 1 };
            let last_month_year = if larger.month() == 1 { larger.year() - 1 } else { larger.year() };
            let days_in_last_month = NaiveDate::from_ymd_opt(last_month_year, last_month, 1)
                .and_then(|_d| NaiveDate::from_ymd_opt(last_month_year, last_month + 1, 1).or_else(|| NaiveDate::from_ymd_opt(last_month_year + 1, 1, 1)))
                .map(|next_month_start| next_month_start.signed_duration_since(NaiveDate::from_ymd_opt(last_month_year, last_month, 1).unwrap()).num_days())
                .unwrap_or(30);
        ...
        let sign = if op.unwrap_or("add".to_string()) == "add" { 1 } else { -1 };
...
fn get_data_transfer_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("bps", 1.0), ("kbps", 1000.0), ("mbps", 1e6), ("gbps", 1e9), ("tbps", 1e12),
        ("b_s", 8.0), ("kb_s", 8000.0), ("mb_s", 8e6), ("gb_s", 8e9), ("tb_s", 8e12)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
    Ok(from_f / to_f)
...
fn get_cooking_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("us_tablespoon", 14.7867648), ("us_teaspoon", 4.92892159), ("us_cup", 236.588237), ("us_fluid_ounce", 29.5735296),
        ("imp_tablespoon", 17.758164), ("imp_teaspoon", 5.919388), ("imp_cup", 284.130625), ("imp_fluid_ounce", 28.4130625),
        ("dash", 0.616115), ("pinch", 0.308058), ("drop", 0.05), ("minim", 0.0616115), ("smidgen", 0.154029), ("tad", 1.23223)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
    Ok(from_f / to_f)
...
fn convert_fuel(value: f64, from: &str, to: &str) -> Result<ConvertResult, String> {
    ...
    let units = std::collections::HashMap::from([
        ("kml", 1.0), ("mpgu", 0.425144), ("mpgk", 0.354006), ("mpl", 1.60934)
    ]);

    // Convert to base (km/L)
    let base_v = if from == "l100km" {
        if value <= 0.0 { return Err("Invalid L/100km value".to_string()); }
        100.0 / value
    } else {
        let f = units.get(from.as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
        value * f
    };

    // Convert from base to target
    let result = if to == "l100km" {
        if base_v <= 0.0 { return Err("Invalid km/L equivalent for L/100km".to_string()); }
        100.0 / base_v
    } else {
        let f = units.get(to.as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
        base_v / f
    };

    Ok(ConvertResult {
        result,
        formatted: format!("{:.10}", result).trim_end_matches('0').trim_end_matches('.').to_string(),
        factor: 1.0, 
    })
}
...
fn get_torque_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("nm", 1.0), ("lbf_ft", 1.355818), ("lbf_in", 0.112985), ("kgf_m", 9.80665), ("ozf_in", 0.007062)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
    Ok(from_f / to_f)
...
fn get_illuminance_factor(from: &str, to: &str) -> Result<f64, String> {
    let units = std::collections::HashMap::from([
        ("lux", 1.0), ("foot_candle", 10.76391), ("phot", 10000.0), ("nox", 0.001), ("candlepower", 1.0)
    ]);
    let from_f = units.get(from.to_lowercase().as_str()).ok_or_else(|| format!("Invalid from unit: {}", from))?;
    let to_f = units.get(to.to_lowercase().as_str()).ok_or_else(|| format!("Invalid to unit: {}", to))?;
    Ok(from_f / to_f)
...
pub fn calculate_ppf(
    ...
    Ok(PPFResult {
        ...
        total_interest: Decimal::from_f64(opening_balance - total_invested).unwrap().round_dp(0),
        yearly_breakup,
    })
}
...
pub fn calculate_nps(
    ...
    Ok(NPSResult {
        ...
        interest_earned: Decimal::from_f64(total_corpus_f64 - total_invested_f64).unwrap().round_dp(0),
...
pub fn solve_cubic(a: f64, b: f64, c: f64, d: f64) -> Result<CubicResult, String> {
    if a == 0.0 {
        return solve_quadratic(b, c, d).map(|r| CubicResult {
            roots: vec![
                Root { real: r.x1_real, imag: r.x1_imag },
                Root { real: r.x2_real, imag: r.x2_imag },
            ]
        });
    }
    ...
    if det > 0.0 {
        ...
        roots.push(Root { real: -aa / 3.0 + (s + t), imag: 0.0 });
        roots.push(Root { real: -aa / 3.0 - (s + t) / 2.0, imag: (s - t) * (3.0f64.sqrt() / 2.0) });
        roots.push(Root { real: -aa / 3.0 - (s + t) / 2.0, imag: -(s - t) * (3.0f64.sqrt() / 2.0) });
    } else if det == 0.0 {
        let r_cbrt = r.cbrt();
        roots.push(Root { real: -aa / 3.0 + 2.0 * r_cbrt, imag: 0.0 });
        roots.push(Root { real: -aa / 3.0 - r_cbrt, imag: 0.0 });
        roots.push(Root { real: -aa / 3.0 - r_cbrt, imag: 0.0 });
    } else {
        let theta = (r / (-q * q * q).sqrt()).acos();
        let q_sqrt = (-q).sqrt();
        roots.push(Root { real: 2.0 * q_sqrt * (theta / 3.0).cos() - aa / 3.0, imag: 0.0 });
        roots.push(Root { real: 2.0 * q_sqrt * ((theta + 2.0 * std::f64::consts::PI) / 3.0).cos() - aa / 3.0, imag: 0.0 });
        roots.push(Root { real: 2.0 * q_sqrt * ((theta + 4.0 * std::f64::consts::PI) / 3.0).cos() - aa / 3.0, imag: 0.0 });
    }
...
pub fn solve_quartic(a: f64, b: f64, c: f64, d: f64, e: f64) -> Result<QuarticResult, String> {
    ...
    if b == 0.0 && d == 0.0 {
        ...
        roots.push(Root { real: r1 * theta1.cos(), imag: r1 * theta1.sin() });
        roots.push(Root { real: -r1 * theta1.cos(), imag: -r1 * theta1.sin() });
        ...
        roots.push(Root { real: r2 * theta2.cos(), imag: r2 * theta2.sin() });
        roots.push(Root { real: -r2 * theta2.cos(), imag: -r2 * theta2.sin() });
...
pub fn solve_quadratic(a: f64, b: f64, c: f64) -> Result<QuadraticResult, String> {
    ...
    if d >= 0.0 {
        ...
        Ok(QuadraticResult {
            x1_real: x1, x1_imag: 0.0,
            x2_real: x2, x2_imag: 0.0,
            is_complex: false,
            discriminant: d,
        })
    } else {
...
pub fn calculate_matrix_ops(
    ...
    match op.as_str() {
        ...
        "mul" => {
            ...
            if a_cols != b_rows {
                return Err("Matrix A columns must equal Matrix B rows for multiplication".to_string());
            }
        ...
        "inverse" => {
            ...
            if size == 2 {
                ...
                if det == 0.0 { return Err("Matrix is singular (det=0)".to_string()); }
                let res = vec![
...
pub fn calculate_bmi(weight: f64, height_cm: f64) -> Result<BMIResult, String> {
    if height_cm <= 0.0 { return Err("Height must be greater than zero".to_string()); }
...
pub fn calculate_marks(marks: Vec<f64>, max_marks: Vec<f64>) -> Result<MarksResult, String> {
    ...
    let grade = if percentage >= 90.0 { "A+" }
    else if percentage >= 80.0 { "A" }
    else if percentage >= 70.0 { "B" }
    else if percentage >= 60.0 { "C" }
    else if percentage >= 50.0 { "D" }
    else { "F" };
...
#[tauri::command]
pub fn calculate_investment_comparison(req: InvestmentComparisonRequest) -> Result<InvestmentComparisonResult, String> {
    let mut data = Vec::new();
    let total_invested = if req.is_monthly { req.amount * 12.0 * req.years } else { req.amount };
    ...
    for i in 0..=(req.years as u32) {
        ...
        for s in &req.scenarios {
            values.insert(s.name.clone(), calculate_fv(s.rate, req.amount, i as f64, req.is_monthly));
        }
...
pub fn calculate_price_comparison(items: Vec<PriceItem>) -> Result<PriceComparisonResult, String> {
    if items.is_empty() { return Err("No items provided".to_string()); }
    
    let mut results: Vec<PriceItemResult> = items.iter().map(|item| {
        let unit_price = if item.quantity > 0.0 { item.price / item.quantity } else { 0.0 };
        PriceItemResult {
    ...
    let min_unit_price = results.iter()
...
pub fn calculate_real_return(
    ...
    Ok(RealReturnResult {
        ...
        future_value_nominal: Decimal::from_f64(fv_nominal).unwrap_or(Decimal::ZERO).round_dp(2),
        future_value_real: Decimal::from_f64(fv_real).unwrap_or(Decimal::ZERO).round_dp(2),
    })
}
...
#[tauri::command]
pub fn calculate_income_tax_india(income: f64, business_income: f64, deductions: f64) -> Result<TaxResult, String> {
    ...
    if taxable_new > 300000.0 {
        if taxable_new <= 700000.0 { tax_new = (taxable_new - 300000.0) * 0.05; }
        else if taxable_new <= 1000000.0 { tax_new = 20000.0 + (taxable_new - 700000.0) * 0.10; }
        else if taxable_new <= 1200000.0 { tax_new = 50000.0 + (taxable_new - 1000000.0) * 0.15; }
        else if taxable_new <= 1500000.0 { tax_new = 80000.0 + (taxable_new - 1200000.0) * 0.20; }
        else { tax_new = 140000.0 + (taxable_new - 1500000.0) * 0.30; }
    }
    
    // Rebate u/s 87A for New Regime (up to 7L income)
    if total_income <= 700000.0 { tax_new = 0.0; }
    ...
    if taxable_old > 250000.0 {
        if taxable_old <= 500000.0 { tax_old = (taxable_old - 250000.0) * 0.05; }
        else if taxable_old <= 1000000.0 { tax_old = 12500.0 + (taxable_old - 500000.0) * 0.20; }
        else { tax_old = 112500.0 + (taxable_old - 1000000.0) * 0.30; }
        
        // Rebate u/s 87A for Old Regime (up to 5L taxable)
        if taxable_old <= 500000.0 { tax_old = 0.0; }
    }
    tax_old *= 1.04; // Cess 4%
...
pub fn calculate_fire_projection(
    ...
    if years <= 0 { return Err("Target age must be greater than current age".to_string()); }
...
pub fn calculate_stock_average(entries: Vec<StockEntry>) -> Result<StockAvgResult, String> {
    ...
    if total_qty == 0.0 { return Err("Total quantity cannot be zero".to_string()); }
    Ok(StockAvgResult {
        total_qty,
...
pub fn calculate_function_samples(
    ...
    let exprs: Vec<&str> = expression.split(';').map(|s| s.trim()).filter(|s| !s.is_empty()).collect();
    ...
    Ok(GraphResult { functions: all_results })
}
